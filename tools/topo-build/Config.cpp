...
void printUsage(const char* argv0) {
    std::cerr << "Usage: " << argv0 << " [<cpp-sources...> --topo <root.topo> -o <output>] [options]\n"
              ...
              << "  --no-check            Skip topo-check (overrides [build].check; warns when an\n"
              << "                        enabled optimization consumes declarations)\n"
              << "\nBy default ([build].check = \"auto\"), topo-check runs on EVERY build and a\n"
              << "check failure fails the build; the incremental check cache keeps no-change\n"
              << "rebuilds near zero cost. Skip it explicitly with --no-check (or\n"
              << "[build].check = \"off\") — skipping while an enabled optimization consumes\n"
              << ".topo declarations (parallel / loop-parallel / lifetime / forced pipeline)\n"
              << "emits a loud UNVERIFIED warning.\n"
              << "\nExamples:\n"
